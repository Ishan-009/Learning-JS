...
//  Using new way to make a map which is more efficient then using set again and again
...
// converting objects to map by using object.entries which convertes object values to the key value type values form that can be used to create an map or convert to map from an object

// In short, we converted object into key value pair using object.enrties adn then used it to create map with defualt map syntax and then we iterated to map and print the result

const exampleObject = {
  name: "Example Object",
  value: "Example Object Value",
};

const sampleMap = new Map([Object.entries(exampleObject)]);
console.log("samplemap", sampleMap);

// Iterating sample map
for (let [key, value] of sampleMap) {
...
for (const [key, value] of iterationMap) {
  if (key !== true && key !== false && key !== "correct") {
    console.log(key, value);
  }
}
